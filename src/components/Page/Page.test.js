...
describe('Page', () => {
  ...
  it('renders page correctly with all props', () => {
    ...
    expect(getByText(title).attributes.getNamedItem('class').value).toBe('Page-title');
    expect(container.firstChild.firstChild.children[0].tagName).toBe('H1');
    expect(container.firstChild.firstChild.children[0].attributes.getNamedItem('class').value).toBe('Page-title');
    expect(container.firstChild.firstChild.children[1].attributes.getNamedItem('class').value).toBe('Page-body');
    expect(getAllByText('children test section')).toHaveLength(2);
    expect(container.firstChild.firstChild.children[1].children).toHaveLength(2);
    expect(container.firstChild.firstChild.children[1].children[0].tagName).toBe('SECTION');
    expect(container.firstChild.firstChild.children[1].children[1].tagName).toBe('SECTION');
  });
  ...
  it('renders page correctly with just children', () => {
    ...
    expect(container.firstChild.firstChild.firstChild.children[0].tagName).toBe('SECTION');
    expect(container.firstChild.firstChild.firstChild.children[1].tagName).toBe('SECTION');
  });
