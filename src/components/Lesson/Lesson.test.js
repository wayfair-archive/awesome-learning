...
describe('Lesson', () => {
  it('renders back to course button', () => {
    const { queryByText } = render(<LastLessonProvider><Lesson lesson={lessonExternalQuiz} slug={lessonExternalQuiz.fields.slug}/></LastLessonProvider>);
  ...
  it('renders the video iframes', () => {
    const { queryByTestId } = render(<LastLessonProvider><Lesson lesson={lessonExternalQuiz} slug={lessonExternalQuiz.fields.slug}/></LastLessonProvider>);
    expect(queryByTestId(lessonExternalQuiz.frontmatter.videoLinks[0])).toBeTruthy();
  });

  it('renders the reading links', () => {
    const { queryByText } = render(<LastLessonProvider><Lesson lesson={lessonExternalQuiz} slug={lessonExternalQuiz.fields.slug}/></LastLessonProvider>);
    expect(queryByText(lessonExternalQuiz.frontmatter.readingLinks[0].title)).toBeTruthy();
    expect(queryByText(lessonExternalQuiz.frontmatter.readingLinks[1].title)).toBeTruthy();
    expect(queryByText(lessonExternalQuiz.frontmatter.readingLinks[0].description, {exact: false})).toBeTruthy();
    expect(queryByText(lessonExternalQuiz.frontmatter.readingLinks[1].description, {exact: false})).toBeTruthy();
  });
  ...
  it('renders a link to external lesson if it exists', () => {
    const {getByText, getByTestId} = render(<LastLessonProvider><Lesson lesson={lessonWithSecondaryUrlAndQuiz} slug={lessonWithSecondaryUrlAndQuiz.fields.slug}/></LastLessonProvider>);
